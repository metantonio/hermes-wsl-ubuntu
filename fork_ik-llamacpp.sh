...
if [ "$start_llama" == "y" ]; then
    ...
    if [ ${#models[@]} -eq 0 ]; then
        ...
        if [[ "$model_idx" =~ ^[0-9]+$ ]] && [ "$model_idx" -ge 1 ] && [ "$model_idx" -le "${#models[@]}" ]; then
            ...
            $AI_OPT_DIR/build/bin/llama-server --model "$SELECTED_MODEL" -ngl 99 --n-cpu-moe 99 -c $CTX_SIZE -np 1 -fa on -ctx $CACHE_TYPE -ctv $CACHE_TYPE $MOE_FLAG $THREADS_FLAG -tb 24 --no-warmup $DRAFT_FLAGS --metrics --host 127.0.0.1 > llama-server.log 2>&1 &
